...
export type CompoundTExp = ProcTExp | TupleTExp | ListTExp;
export const isCompoundTExp = (x: any): x is CompoundTExp => isProcTExp(x) || isTupleTExp(x) || isListTExp(x);

export type NonTupleTExp = AtomicTExp | ProcTExp | TVar | ListTExp;
export const isNonTupleTExp = (x: any): x is NonTupleTExp =>
    isAtomicTExp(x) || isProcTExp(x) || isTVar(x) || isListTExp(x);
...
// Homogenous Lists
export type ListTExp = { tag: "ListTExp" ; itemTE: TExp };
export const makeListTExp = (itemTE: TExp): ListTExp =>
    ({tag:"ListTExp", itemTE: itemTE});
export const isListTExp = (x:any): x is ListTExp =>
    x.tag === "ListTExp";
...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => {
    ...
    return (pos === -1) ?
            (texps.length === 0 ?
            makeFailure("Empty compound type")
            : texps.length > 2 && texps[0] === "list" ?
            makeFailure("List with more than one parameter type")
            : texps.length === 2 && texps[0] === "list" ?
            mapv(parseTExp(texps[1]), makeListTExp)
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isProcTExp(x) ? bind(unparseTuple(x.paramTEs), (paramTEs: string[]) =>
                            mapv(unparseTExp(x.returnTE), (returnTE: string) =>
                                [...paramTEs, '->', returnTE])) :
        isListTExp(x) ? mapv(unparseTExp(x.itemTE), (itemTE:string) =>
            `(list ${itemTE})`) :
