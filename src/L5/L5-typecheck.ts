...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): Result<VoidTExp> =>
    bind(typeofExp(exp.val, tenv), (typeOfVal: TExp) =>
        bind(checkEqualType(exp.var.texp, typeOfVal, exp), _ =>
            makeOk(makeVoidTExp())));
...
const extendTEnvWithDefine = (def: DefineExp, tenv: TEnv): TEnv =>
    makeExtendTEnv(
        [def.var.var],
        [def.var.texp],
        tenv
...
const typeofProgramExps = (exps: List<Exp>, tenv: TEnv): Result<TExp> => {
    ...
    const current = first(exps);
    const remaining = rest(exps);
    if (isEmpty(remaining)) {
        return typeofExp(current, tenv);
    }
    if (isDefineExp(current)) {
        return bind(typeofDefine(current, tenv), _ =>
            typeofProgramExps(
                remaining,
                extendTEnvWithDefine(current, tenv)
            )
        );
    }
    return bind(typeofExp(current, tenv), _ =>
        typeofProgramExps(remaining, tenv)
    );
};
